...
async def thor_ops(network, unit_asset):
    ...
    expected = thor.estimate_swap_output(in_amount=0.01, in_asset=unit_asset, out_asset=Asset.from_str("BNB.BUSD-74E"))
    print(expected)
    hash = await thor.swap(in_amount=0.01, in_asset=unit_asset, out_asset=Asset.from_str("BNB.BUSD-74E"),
                           wait=False)
    print(hash)
    for b in await thor.account.bnb.get_balance(address=bnb_add):
        print(f"{b.asset} : {b.amount}")
...
def main():
    rune = {'network': 'MCTN', 'unit_asset':'ETH.ETH'}

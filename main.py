...
async def thor_ops(network, unit_asset, trading_asset, cex_oracle, diff):
    ...
    # Trading
    thor = THORTrader(network=network, host=["136.244.65.26","159.65.212.234", "174.138.105.79"])
    # Account Statement
    ...
    # withdraw timer
    last_withdraw = 0
    cex_balance = await cex_oracle.get_balance(symbol="ETH")
    thor_balance = await thor.account.get_balance(asset=Asset.from_str("BNB.BUSD-BD1"))
    MONGO.post_balance({"eth": float(cex_balance), "BUSD": float(thor_balance), "nonce": 1})
    while found == 0:
        time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    profile = {'network': 'MCCN', 'unit_asset': 'BNB.BUSD-BD1',
               'trading_asset': ['ETH.ETH'], 'cex_oracle': ftx,
               'diff': 2}

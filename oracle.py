...
class ThorOracle:
    ...
    def get_network_fee(self, in_coin, out_coin):
        ...
        if out_coin.chain == 'THOR':
            ...
            if 'ETH' in out_coin.symbol:
                ...
                gas_fee_in_eth = gasLimit * self.get_gas_rate(chain=out_coin.chain) * 3 / 10 ** 9  # RETURN IN ETH
                network_fee = gas_fee_in_eth
            else:
                gasLimit = self.GAS_MULTIPLIER['ERC-20']
                gas_fee_in_eth = gasLimit * self.get_gas_rate(chain=out_coin.chain) * 3 / 10**9
            ...
            gas_fee_in_bnb = gasLimit * self.get_gas_rate(chain=out_coin.chain) * 3 / 10 ** 8
    ...
    def get_transaction_fee(self, in_coin):
        if in_coin.chain == 'ETH':
            # gasLimit = 21000 + 68 * dataByteLength : formula to calculate gas limit
            if 'ETH' in in_coin.symbol:
                gasLimit = self.GAS_MULTIPLIER['ETH']
                gas_fee_in_eth = gasLimit * self.get_gas_rate(chain=in_coin.chain) / 10 ** 9  # RETURN IN ETH
                tx_fee = gas_fee_in_eth
            else:
                gasLimit = self.GAS_MULTIPLIER['ERC-20']
                gas_fee_in_eth = gasLimit * self.get_gas_rate(chain=in_coin.chain) / 10**9
                ## Using USD value Output
                alt_ether_weigh = self.get_fiat_price(in_coin) / self.get_fiat_price('ETH.ETH')
                gas_fee_in_alt = gas_fee_in_eth / alt_ether_weigh
                tx_fee = gas_fee_in_alt
        elif in_coin.chain == 'THOR':
            tx_fee = 0.02
        elif in_coin.chain == 'BNB':
            gasLimit = self.GAS_MULTIPLIER['BNB']
            gas_fee_in_bnb = gasLimit * self.get_gas_rate(chain=in_coin.chain) / 10 ** 8
            tx_fee = gas_fee_in_bnb
            if 'BNB' not in in_coin.symbol:
                alt_bnb_weigh = self.get_fiat_price(in_coin) / self.get_fiat_price('BNB.BNB')
                gas_fee_in_alt = gas_fee_in_bnb / alt_bnb_weigh
                tx_fee = gas_fee_in_alt
        return tx_fee
    ...
    def get_thornode_tx_detail(self, tx_id, block_time=1, timeout=300):
        ...
        while i < timeout:
            try:
                ...
                if DEBUGGING:
                    tx_detail = requests.get(f'https://testnet.thornode.thorchain.info/thorchain/tx/{tx_id}').json()
                else:
                    tx_detail = requests.get(f'https://thornode.thorchain.info/thorchain/tx/{tx_id}').json()
                if tx_detail != None and tx_detail != {'error':'internal'}:
